...
const ProfileSideBar: React.FC = () => {
  ...
  const handleMenuClick = (label: string) => {
    ...
    if (window.innerWidth < 640) { // sm breakpoint, typically 640px
      setIsOpen(false);
    }
  };

  return (
    <div className="relative  min-w-[50%]  sm:min-w-[30%] lg:min-w-[20%] xl:min-w-[15%]">
      {!isOpen && (
        <button 
      ...
      <nav className={`absolute sm:relative top-0 transition-all h-full w-full   sm:flex items-center justify-center xl:h-[520px] w-full  text-black duration-300 ease-in-out ${isOpen ? 'left-0' : '-left-64'}`}>
        ...
        <button
          onClick={toggleSidebar}
          className="absolute sm:hidden top-2 right-2 text-gray-500 hover:text-gray-700 border rounded-full bg-gray-200 p-2"
